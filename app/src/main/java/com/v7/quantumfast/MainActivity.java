...
public class MainActivity extends Activity {
 ...
 EditText searchApps; EditText searchWeb;
    ...
    RecyclerView rvSuggestions, rvFav, rvFolders; List<ResolveInfo> suggList = new ArrayList<>(); SuggAdapter suggAd;
...
protected void onCreate(Bundle b){
 ...
 registerBatteryReceiver();
        ...
        rvSuggestions=findViewById(R.id.rvSuggestionsestions); rvFav=findViewById(R.id.rvFavorites); rvFolders=findViewById(R.id.rvFolders);
        rvSuggestions.setHasFixedSize(true); rvSuggestions.setItemAnimator(null); rvFav.setHasFixedSize(true); rvFav.setItemAnimator(null); rvFolders.setHasFixedSize(true); rvFolders.setItemAnimator(null);
        rvSuggestions.setLayoutManager(new LinearLayoutManager(this)); suggAd=new SuggAdapter(); rvSuggestions.setAdapter(suggAd);
        ...
        searchApps.addTextChangedListener(new android.text.TextWatcher(){
            ...
            public void onTextChanged(CharSequence q,int bb,int cc,int dd){
                ...
                debounce[0]=()->{
                    String qq=q.toString().trim(); if(qq.isEmpty()){ main.post(()->{ rvSuggestions.setVisibility(View.GONE); clear.setVisibility(View.GONE); suggList.clear(); if(!rvSuggestions.isComputingLayout()) suggAd.notifyDataSetChanged(); }); return; }
                    ...
                    for(ResolveInfo ri:snap){ String lbl=QuantumApp.labelCache.get(ri.activityInfo.packageName); if(lbl!=null && lbl.toLowerCase().contains(low)){ r.add(ri); if(r.size()>=12) break; } }
                    ...
                    main.post(()->{ clear.setVisibility(View.VISIBLE); suggList.clear(); suggList.addAll(r); rvSuggestions.setVisibility(r.isEmpty()?View.GONE:View.VISIBLE); if(!rvSuggestions.isComputingLayout()) suggAd.notifyDataSetChanged(); });
        ...
        rv.setAdapter(new androidx.recyclerview.widget.RecyclerView.Adapter(){
            ...
            public void onBindViewHolder(androidx.recyclerview.widget.RecyclerView.ViewHolder hh,int pos){
                try{
                    ...
                    String lbl=QuantumApp.labelCache.get(ri.activityInfo.packageName); if(lbl!=null) h.lb.setText(lbl);
                    android.graphics.drawable.android.graphics.Bitmap b=QuantumApp.QuantumApp.iconCache.get(ri.activityInfo.packageName);
                    if(b!=null) h.ic.setImageBitmap(b);
                    else {
    ...
    void boostAllApps(){
        try{
            ...
            try{
                ...
                ((androidx.recyclerview.widget.LinearLayoutManager)rvFolders.getLayoutManager()).setInitialPrefetchItemCount(100);
            ...
            try{
                ...
                ((androidx.recyclerview.widget.LinearLayoutManager)rvSuggestions.getLayoutManager()).setInitialPrefetchItemCount(100);
