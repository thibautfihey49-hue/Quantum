...
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle s){
        ...
        try{
            ...
            loadFavs(); setupClock(); setupDockSimple(); setupFavsSafe(); setupListeners(); setupWidgetHost();
            registerAutoDetect();
                ...
                public boolean onTouch(View v, MotionEvent ev){
                    switch(ev.getAction()){
                        case MotionEvent.ACTION_DOWN:
                            dX = getX() - ev.getRawX(); dY = getY() - ev.getRawY(); lastAction=MotionEvent.ACTION_DOWN; bringToFront(); return true;
                        case MotionEvent.ACTION_MOVE:
                            setX(ev.getRawX()+dX); setY(ev.getRawY()+dY); lastAction=MotionEvent.ACTION_MOVE; return true;
                        case MotionEvent.ACTION_UP:
                            if(lastAction==MotionEvent.ACTION_MOVE) glassPrefs.edit().putFloat("widget_"+widgetId+"_x", getX()).putFloat("widget_"+widgetId+"_y", getY()).apply();
                            else showWidgetOptions(widgetId);
                            return true;
                    }
                    return false;
                }
    ...
    void showWidgetOptions(int id){
        new AlertDialog.Builder(this).setTitle("Widget").setItems(new String[]{"Redimensionner","Supprimer"}, (d,w)->{
            if(w==0) showResizeDialog(id); else removeWidget(id);
        }).show();
    }
    void showResizeDialog(int id){
        try{
            android.appwidget.AppWidgetProviderInfo info=widgetManager.getAppWidgetInfo(id); if(info==null) return;
            LinearLayout lay=new LinearLayout(this); lay.setOrientation(LinearLayout.VERTICAL); lay.setPadding(30,20,30,20);
            SeekBar wBar=new SeekBar(this); wBar.setMax(600); wBar.setProgress((int)glassPrefs.getFloat("widget_"+id+"_w", info.minWidth)); lay.addView(wBar);
            SeekBar hBar=new SeekBar(this); hBar.setMax(900); hBar.setProgress((int)glassPrefs.getFloat("widget_"+id+"_h", info.minHeight)); lay.addView(hBar);
            new AlertDialog.Builder(this).setTitle("Redimensionner").setView(lay).setPositiveButton("OK",(dd,ww)->{
                int nw=wBar.getProgress(); int nh=hBar.getProgress();
                glassPrefs.edit().putFloat("widget_"+id+"_w", nw).putFloat("widget_"+id+"_h", nh).apply();
                Bundle opts=new Bundle(); opts.putInt(android.appwidget.AppWidgetManager.OPTION_APPWIDGET_MIN_WIDTH, nw); opts.putInt(android.appwidget.AppWidgetManager.OPTION_APPWIDGET_MIN_HEIGHT, nh);
                widgetManager.updateAppWidgetOptions(id, opts); loadWidgets();
            }).show();
        }catch(Exception e){}
    }
    ...
    void loadWidgets(){
        ...
        String saved=glassPrefs.getString("widget_ids",""); if(saved.isEmpty()) return;
        for(String s: saved.split(",")){
            ...
            try{
                ...
                if(info!=null){
                    ...
                    frame.enableDrag();
                    float x=glassPrefs.getFloat("widget_"+id+"_x", -1); float y=glassPrefs.getFloat("widget_"+id+"_y", -1);
                    if(x!=-1) frame.setX(x); if(y!=-1) frame.setY(y);
    ...
    void removeWidget(int id){
        try{ widgetHost.deleteAppWidgetId(id); String cur=glassPrefs.getString("widget_ids",""); List<String> list=new ArrayList<>(Arrays.asList(cur.split(","))); list.remove(String.valueOf(id)); list.remove(""); glassPrefs.edit().putString("widget_ids", String.join(",", list)).apply(); loadWidgets(); }catch(Exception e){}
    }
    void saveWidgetId(int id){ String cur=glassPrefs.getString("widget_ids",""); Set<String> set=new HashSet<>(Arrays.asList(cur.split(","))); set.add(String.valueOf(id)); set.remove(""); glassPrefs.edit().putString("widget_ids", String.join(",", set)).apply(); }
