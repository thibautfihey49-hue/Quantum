...
import android.app.Activity; import android.content.*; import android.content.pm.*; import android.graphics.*; import android.graphics.drawable.Drawable; import android.net.Uri; import android.os.*; import android.view.*; import android.view.inputmethod.EditorInfo; import android.widget.*; import androidx.recyclerview.widget.*; import java.io.InputStream; import java.text.SimpleDateFormat; import java.util.*; import java.util.concurrent.*;
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){
        ...
        // FIX CRASH : plus de pool partagé, animators null, cache 10
        rvSugg.setHasFixedSize(true); rvSugg.setItemAnimator(null); rvSugg.setItemViewCacheSize(10); rvSugg.setNestedScrollingEnabled(false);
        rvFav.setHasFixedSize(true); rvFav.setItemAnimator(null); rvFav.setItemViewCacheSize(10); rvFav.setNestedScrollingEnabled(false);
        ...
        preloadFast(); clock(); setupDock(); loadWallpaperFast();
        ...
        searchApps.addTextChangedListener(new android.text.TextWatcher(){
            ...
            public void onTextChanged(CharSequence q,int a,int b,int c){
                ...
                debounce[0]=()->{
                    String qq=q.toString().trim();
                    if(qq.isEmpty()){
                        main.post(()->{
                            rvSugg.setVisibility(View.GONE); clear.setVisibility(View.GONE);
                            suggList.clear();
                            if(!rvSugg.isComputingLayout()) suggAd.notifyDataSetChanged();
                        });
                        return;
                    }
                    List<ResolveInfo> snapshot;
                    synchronized(cache){ snapshot=new ArrayList<>(cache); }
                    List<ResolveInfo> r=new ArrayList<>();
                    String low=qq.toLowerCase();
                    for(ResolveInfo ri:snapshot){ if(ri.loadLabel(getPackageManager()).toString().toLowerCase().contains(low)){ r.add(ri); if(r.size()>=12) break; } }
                    main.post(()->{
                        clear.setVisibility(View.VISIBLE);
                        suggList.clear(); suggList.addAll(r);
                        rvSugg.setVisibility(r.isEmpty()?View.GONE:View.VISIBLE);
                        if(!rvSugg.isComputingLayout()) suggAd.notifyDataSetChanged(); else rvSugg.post(()->suggAd.notifyDataSetChanged());
                    });
                };
                main.postDelayed(debounce[0], 80);
            }
        });
    ...
    void loadWallpaperFast(){
        String uriStr=prefs.getString("custom_wallpaper_uri",null); if(uriStr==null) return;
        exec.execute(()->{
            try{
                Uri uri=Uri.parse(uriStr);
                InputStream is=getContentResolver().openInputStream(uri);
                if(is==null) return;
                BitmapFactory.Options opts=new BitmapFactory.Options(); opts.inJustDecodeBounds=true; BitmapFactory.decodeStream(is,null,opts); is.close();
                int reqW=getResources().getDisplayMetrics().widthPixels; int reqH=getResources().getDisplayMetrics().heightPixels;
                int sample=1; while(opts.outWidth/sample/2>=reqW && opts.outHeight/sample/2>=reqH) sample*=2;
                InputStream is2=getContentResolver().openInputStream(uri); if(is2==null) return;
                BitmapFactory.Options o2=new BitmapFactory.Options(); o2.inSampleSize=sample; o2.inPreferredConfig=Bitmap.Config.RGB_565;
                Bitmap bmp=BitmapFactory.decodeStream(is2,null,o2); is2.close();
                main.post(()->{ if(bmp!=null &&!bmp.isRecycled()) wallpaperView.setImageBitmap(bmp); });
            }catch(Exception e){ }
        });
    }
    ...
    void updateDockIcon(ImageView iv, String pkg){ Drawable cd=iconCache.get(pkg); if(cd!=null){ iv.setImageDrawable(cd); return; } exec.execute(()->{ try{ Drawable d=getPackageManager().getApplicationIcon(pkg); if(d!=null){ iconCache.put(pkg,d); main.post(()->{ try{ iv.setImageDrawable(d); }catch(Exception e){} }); } }catch(Exception e){}}); }
    void pickDockApp(int dockIdx){ android.app.Dialog dlg=new android.app.Dialog(this, android.R.style.Theme_Translucent_NoTitleBar_Fullscreen); dlg.setContentView(R.layout.picker_dock); RecyclerView rv=dlg.findViewById(R.id.recyclerDock); rv.setHasFixedSize(true); rv.setItemAnimator(null); rv.setLayoutManager(new GridLayoutManager(this,5)); List<ResolveInfo> list; synchronized(cache){ list=new ArrayList<>(cache); } rv.setAdapter(new RecyclerView.Adapter(){ class H extends RecyclerView.ViewHolder{ ImageView ic; TextView lb; H(View v){super(v); ic=v.findViewById(R.id.icon); lb=v.findViewById(R.id.label);} } public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup p,int t){ return new H(getLayoutInflater().inflate(R.layout.item_app,p,false)); } public void onBindViewHolder(RecyclerView.ViewHolder hh,int pos){ try{ H h=(H)hh; if(pos>=list.size()) return; ResolveInfo ri=list.get(pos); h.lb.setText(ri.loadLabel(getPackageManager())); Drawable d=iconCache.get(ri.activityInfo.packageName); if(d!=null) h.ic.setImageDrawable(d); else { h.ic.setImageResource(android.R.drawable.sym_def_app_icon); exec.execute(()->{ try{ Drawable dd=getPackageManager().getApplicationIcon(ri.activityInfo.packageName); iconCache.put(ri.activityInfo.packageName, dd); main.post(()->{ if(h.getBindingAdapterPosition()==pos) h.ic.setImageDrawable(dd); }); }catch(Exception e){} }); } h.itemView.setOnClickListener(v->{ prefs.edit().putString(dockKeys[dockIdx], ri.activityInfo.packageName).apply(); dlg.dismiss(); setupDock(); }); }catch(Exception e){} } public int getItemCount(){ return list.size(); } }); dlg.show(); }
    void showBrowserChooser(String query){ String url = query.startsWith("http")? query : "https://www.google.com/search?q="+Uri.encode(query); Intent base = new Intent(Intent.ACTION_VIEW, Uri.parse(url)); List<ResolveInfo> browsers = getPackageManager().queryIntentActivities(base, 0); if(browsers.isEmpty()) return; String[] names=new String[browsers.size()]; for(int i=0;i<browsers.size();i++) names[i]=browsers.get(i).loadLabel(getPackageManager()).toString(); new android.app.AlertDialog.Builder(this).setTitle("Ouvrir: "+query).setItems(names,(d,which)->{ try{ ResolveInfo ri=browsers.get(which); Intent intent=new Intent(Intent.ACTION_VIEW, Uri.parse(url)); intent.setPackage(ri.activityInfo.packageName); startActivity(intent);}catch(Exception e){} }).show(); }
    void openDrawerWithQuery(String initial){ if(!tap() && (initial==null || initial.isEmpty())) return; android.app.Dialog dlg=new android.app.Dialog(this,android.R.style.Theme_Translucent_NoTitleBar_Fullscreen); dlg.getWindow().setStatusBarColor(0); dlg.getWindow().setNavigationBarColor(0); dlg.setContentView(R.layout.drawer); RecyclerView rv=dlg.findViewById(R.id.recycler); EditText s=dlg.findViewById(R.id.search); rv.setHasFixedSize(true); rv.setItemAnimator(null); rv.setItemViewCacheSize(20); rv.setLayoutManager(new GridLayoutManager(this,5)); final List<ResolveInfo> filt = new ArrayList<>(); if(initial==null) initial=""; String lowInit=initial.toLowerCase(); synchronized(cache){ if(lowInit.isEmpty()) filt.addAll(cache); else for(ResolveInfo ri:cache) try{ if(ri.loadLabel(getPackageManager()).toString().toLowerCase().contains(lowInit)) filt.add(ri); }catch(Exception e){} } final FastAdapter ad = new FastAdapter(filt,getPackageManager(),dlg); rv.setAdapter(ad); s.setText(initial); final Runnable[] runHolder = new Runnable[1]; s.addTextChangedListener(new android.text.TextWatcher(){ public void beforeTextChanged(CharSequence a,int b,int c,int d){} public void afterTextChanged(android.text.Editable e){} public void onTextChanged(CharSequence q,int b,int c,int dd){ if(runHolder[0]!=null) main.removeCallbacks(runHolder[0]); runHolder[0]=()->{ String qq=q.toString().toLowerCase().trim(); List<ResolveInfo> r=new ArrayList<>(); List<ResolveInfo> snap; synchronized(cache){ snap=new ArrayList<>(cache); } if(qq.isEmpty()) r.addAll(snap); else for(ResolveInfo ri:snap) try{ if(ri.loadLabel(getPackageManager()).toString().toLowerCase().contains(qq)) r.add(ri); }catch(Exception e){} main.post(()->{ filt.clear(); filt.addAll(r); if(!rv.isComputingLayout()) ad.notifyDataSetChanged(); else rv.post(()->ad.notifyDataSetChanged()); }); }; main.postDelayed(runHolder[0],60); } }); dlg.findViewById(R.id.close).setOnClickListener(v->dlg.dismiss()); dlg.show(); }
