...
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){
        ...
        try{ findViewById(R.id.btnDossier).setOnClickListener(v->createFolderDialog()); }catch(Exception e){} try{ findViewById(R.id.Dossier).setOnClickListener(v->createFolderDialog()); }catch(Exception e){} setContentView(R.layout.activity_main);
    ...
    void showBrowserChooserGlass(String query){
        ...
        dlg.try{ findViewById(R.id.btnDossier).setOnClickListener(v->createFolderDialog()); }catch(Exception e){} try{ findViewById(R.id.Dossier).setOnClickListener(v->createFolderDialog()); }catch(Exception e){} setContentView(R.layout.dialog_chooser);
    ...
    void showCreateFolderDialog(){
        android.app.Dialog dlg=new android.app.Dialog(this, android.R.style.Theme_Translucent_NoTitleBar); dlg.try{ findViewById(R.id.btnDossier).setOnClickListener(v->createFolderDialog()); }catch(Exception e){} try{ findViewById(R.id.Dossier).setOnClickListener(v->createFolderDialog()); }catch(Exception e){} setContentView(R.layout.dialog_folder_create);
    ...
    void showFolderContent(Folder f){
        android.app.Dialog dlg=new android.app.Dialog(this, android.R.style.Theme_Translucent_NoTitleBar_Fullscreen); dlg.try{ findViewById(R.id.btnDossier).setOnClickListener(v->createFolderDialog()); }catch(Exception e){} try{ findViewById(R.id.Dossier).setOnClickListener(v->createFolderDialog()); }catch(Exception e){} setContentView(R.layout.picker_dock);
    ...
    class FolderAdapter extends RecyclerView.Adapter<FolderAdapter.H>{
    class H extends RecyclerView.ViewHolder{ TextView name; H(View v){super(v); name=v.findViewById(R.id.favName);} }
    public H onCreateViewHolder(ViewGroup p,int t){ View v=getLayoutInflater().inflate(R.layout.item_fav,p,false); return new H(v); }
    public void onBindViewHolder(H h,int pos){ try{ if(pos>=folders.size()) return; Folder f=folders.get(pos); h.name.setText("📁 "+f.name+" ("+f.pkgs.size()+")"); h.itemView.setOnClickListener(v->showFolderContent(f)); h.itemView.setOnLongClickListener(v->{ folders.remove(pos); saveFolders(); notifyDataSetChanged(); return true; }); }catch(Exception e){} }
...
private void createFolderDialog(){
    ...
    new androidx.appcompat.app.AlertDialog.Builder(this).setTitle("Nouveau dossier").setView(input)
   .setPositiveButton("Créer",(d,w)->{
        String n=input.getText().toString().trim();
        if(!n.isEmpty()){ folders.add(new Folder(n)); saveFolders(); folderAdapter.notifyDataSetChanged(); }
    }).setNegativeButton("Annuler",null).show();
}
void saveFolders(){ getSharedPreferences("quantum",0).edit().putString("folders", new com.google.gson.Gson().toJson(folders)).apply(); }
void loadFolders(){ try{ String j=getSharedPreferences("quantum",0).getString("folders","[]"); java.lang.reflect.Type t=new com.google.gson.reflect.TypeToken<java.util.List<Folder>>(){}.getType(); java.util.List<Folder> l=new com.google.gson.Gson().fromJson(j,t); if(l!=null) folders=l; }catch(Exception e){} }
