...
public class MainActivity extends Activity {
    ...
    class FolderAdapter extends RecyclerView.Adapter<FolderAdapter.H>{ class H extends RecyclerView.ViewHolder{ TextView name; LinearLayout icons; H(View v){super(v); name=v.findViewById(R.id.favName); icons=v.findViewById(R.id.folderIcons);} } public H onCreateViewHolder(ViewGroup p,int t){ View v=getLayoutInflater().inflate(R.layout.item_fav,p,false); LinearLayout ic=v.findViewById(R.id.favIcon).getParent() instanceof LinearLayout? (LinearLayout)v.findViewById(R.id.favIcon).getParent() : null; return new H(v); } public void onBindViewHolder(H h,int pos){ try{ if(pos>=folders.size()) return; Folder f=folders.get(pos); h.name.setText("📁 "+f.name+" ("+f.pkgs.size()+")"); h.itemView.setOnClickListener(v->showFolderContent(f)); h.itemView.setOnLongClickListener(v->{ folders.remove(pos); saveFolders(); notifyDataSetChanged(); return true; }); }catch(Exception e){} } public int getItemCount(){ return folders.size(); } }
