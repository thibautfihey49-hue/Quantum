...
public class MainActivity extends Activity {
 android.appwidget.AppWidgetHost mAppWidgetHost;
 ...
 float dens=getResources().getDisplayMetrics().density; LinearLayout list=new LinearLayout(getDialogContext()); list.setOrientation(LinearLayout.VERTICAL); String[] opts={"🎨 Couleur thème","🖼️ Fond d'écran","🧹 Effacer fond","⭐ Mes apps fusée","🎨 Thèmes d'icônes gratuits","🔤 Polices + Fonds HD","🧩 Widget draggable"}; for(int i=0;i<opts.length;i++){ final int idx=i; TextView row=new TextView(getDialogContext()); row.setText(opts[i]); row.setTextSize(16); row.setTextColor(Color.WHITE); row.setPadding((int)(14*dens),(int)(16*dens),(int)(14*dens),(int)(16*dens)); row.setBackground(glassBg(Color.BLACK, 14*dens, 70)); LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-1,-2); lp.setMargins(0,0,0,(int)(10*dens)); row.setLayoutParams(lp); row.setOnClickListener(v->{ if(idx==0) showPaletteModern(); else if(idx==1) pickWallpaper(); else if(idx==2){ prefs.edit().remove("custom_wallpaper_uri").apply(); android.view.View bg=findV("wallpaper","bg","background","wall"); if(bg instanceof android.widget.ImageView) ((android.widget.ImageView)bg).setImageDrawable(null); } else if(idx==3) showManualTopPicker(); else if(idx==4) showIconPackPicker(); else if(idx==5) showFontsWallpapersPicker(); else if(idx==6) pickWidget(); }); list.addView(row); } AlertDialog dlg=createModernDialog("Quantum Ultra", list); dlg.show(); }
    ...
    void pickWidget(){
        try{
            ...
            android.widget.LinearLayout list = new android.widget.LinearLayout(ctx); list.setOrientation(android.widget.LinearLayout.VERTICAL);
            ...
            for(android.appwidget.AppWidgetProviderInfo info: providers){
                ...
                row.setTextSize(15); row.setTextColor(0xFFFFFFFF); row.setPadding(pad,pad,pad,pad);
                row.setOnClickListener(vv->{
                    try{
                        int id = mAppWidgetHost.allocateAppWidgetId();
                        boolean b = awm.bindAppWidgetIdIfAllowed(id, info.provider);
                        if(!b){ android.content.Intent bi = new android.content.Intent(android.appwidget.AppWidgetManager.ACTION_APPWIDGET_BIND); bi.putExtra(android.appwidget.AppWidgetManager.EXTRA_APPWIDGET_ID,id); bi.putExtra(android.appwidget.AppWidgetManager.EXTRA_APPWIDGET_PROVIDER,info.provider); startActivityForResult(bi,9002); }
                        else{ addWidgetView(id, info); }
                    }catch(Exception e){ android.widget.Toast.makeText(this,"W: "+e.getMessage(),1).show(); }
                });
                list.addView(row);
            }
    ...
    void addWidgetView(int appWidgetId, android.appwidget.AppWidgetProviderInfo info){
        try{ android.appwidget.AppWidgetHostView hv = mAppWidgetHost.createView(this, appWidgetId, info); hv.setAppWidget(appWidgetId, info); android.view.ViewGroup vg = (android.view.ViewGroup) mainRoot; if(vg!=null){ vg.addView(hv); makeWidgetDraggable(hv); } }catch(Exception e){}
    }
    void makeWidgetDraggable(android.view.View v){
        v.setOnTouchListener(new android.view.View.OnTouchListener(){ float dx,dy; public boolean onTouch(android.view.View vv, android.view.MotionEvent ev){ switch(ev.getAction()){ case android.view.MotionEvent.ACTION_DOWN: dx=vv.getX()-ev.getRawX(); dy=vv.getY()-ev.getRawY(); return true; case android.view.MotionEvent.ACTION_MOVE: vv.setX(ev.getRawX()+dx); vv.setY(ev.getRawY()+dy); return true; } return false; } });
    }
    ...
    void showIconPackPicker(){
        try{
            android.content.Context ctx = getDialogContext(); android.widget.LinearLayout list = new android.widget.LinearLayout(ctx); list.setOrientation(android.widget.LinearLayout.VERTICAL);
            int pad = (int)(getResources().getDisplayMetrics().density*12);
            ...
            for(android.content.pm.ResolveInfo ri: packs){ String pkg=ri.activityInfo.packageName; if(!seen.add(pkg)) continue; android.widget.TextView row=new android.widget.TextView(ctx); try{ row.setText(ri.loadLabel(pm)); }catch(Exception e){ row.setText(pkg); } row.setTextSize(15); row.setTextColor(0xFFFFFFFF); row.setPadding(pad,pad,pad,pad); row.setOnClickListener(vv->{ prefs.edit().putString("icon_pack",pkg).apply(); android.widget.Toast.makeText(this,"Pack: "+pkg,0).show(); }); list.addView(row); }
            android.widget.TextView more=new android.widget.TextView(ctx); more.setText("➕ Play Store packs gratuits"); more.setTextColor(0xFF00E5FF); more.setPadding(pad,pad*2,pad,pad); more.setOnClickListener(vv->{ try{ startActivity(new android.content.Intent(android.content.Intent.ACTION_VIEW, android.net.Uri.parse("market://search?q=free icon pack"))); }catch(Exception e){ startActivity(new android.content.Intent(android.content.Intent.ACTION_VIEW, android.net.Uri.parse("https://play.google.com/store/search?q=free%20icon%20pack"))); } }); list.addView(more);
            android.widget.ScrollView sv=new android.widget.ScrollView(ctx); sv.addView(list); createModernDialog("Thèmes d'icônes gratuits", sv).show();
        }catch(Exception e){}
    }
    void showFontsWallpapersPicker(){
        try{
            android.content.Context ctx=getDialogContext(); android.widget.LinearLayout root=new android.widget.LinearLayout(ctx); root.setOrientation(android.widget.LinearLayout.VERTICAL); int pad=(int)(getResources().getDisplayMetrics().density*12);
            android.widget.TextView h1=new android.widget.TextView(ctx); h1.setText("🔤 Polices Google"); h1.setTextColor(0xFFFFFFFF); h1.setPadding(pad,pad,pad,pad/2); root.addView(h1);
            String[][] fonts={{"Poppins","https://github.com/google/fonts/raw/main/ofl/poppins/Poppins-Regular.ttf"},{"Montserrat","https://github.com/google/fonts/raw/main/ofl/montserrat/Montserrat-Regular.ttf"}};
            for(String[] f: fonts){ android.widget.TextView row=new android.widget.TextView(ctx); row.setText("• "+f[0]); row.setTextColor(0xFFFFFFFF); row.setPadding(pad*2,pad/2,pad,pad/2); row.setOnClickListener(vv->{ downloadFont(f[0], f[1]); }); root.addView(row); }
            android.widget.TextView h2=new android.widget.TextView(ctx); h2.setText("\n🖼️ Fonds HD"); h2.setTextColor(0xFFFFFFFF); h2.setPadding(pad,pad,pad,pad/2); root.addView(h2);
            android.widget.TextView rw=new android.widget.TextView(ctx); rw.setText("• Aléatoire HD"); rw.setTextColor(0xFF00E5FF); rw.setPadding(pad*2,pad/2,pad,pad/2); rw.setOnClickListener(vv->{ downloadWallpaper("https://picsum.photos/1080/1920"); }); root.addView(rw);
            android.widget.ScrollView sv=new android.widget.ScrollView(ctx); sv.addView(root); createModernDialog("Polices + Fonds", sv).show();
        }catch(Exception e){}
    }
    void downloadFont(String name, String url){
        new Thread(()->{ try{ java.io.File dir=new java.io.File(getFilesDir(),"fonts"); dir.mkdirs(); java.io.File out=new java.io.File(dir,name+".ttf"); java.io.InputStream in=new java.net.URL(url).openStream(); java.io.FileOutputStream fos=new java.io.FileOutputStream(out); byte[] b=new byte[8192]; int r; while((r=in.read(b))!=-1) fos.write(b,0,r); fos.close(); in.close(); runOnUiThread(()->{ prefs.edit().putString("custom_font_path",out.getAbsolutePath()).apply(); android.widget.Toast.makeText(this,"Police "+name+" OK",0).show(); }); }catch(Exception e){} }).start();
    }
    void downloadWallpaper(String url){
        new Thread(()->{ try{ java.io.InputStream in=new java.net.URL(url).openStream(); android.graphics.Bitmap bmp=android.graphics.BitmapFactory.decodeStream(in); in.close(); runOnUiThread(()->{ try{ android.view.View bg=findV("wallpaper","bg","background","wall"); if(bg instanceof android.widget.ImageView) ((android.widget.ImageView)bg).setImageBitmap(bmp); }catch(Exception ee){} }); }catch(Exception e){} }).start();
    }
 ...
 pool.execute(()->{ try{ Intent it=new Intent(Intent.ACTION_MAIN); it.addCategory(Intent.CATEGORY_LAUNCHER); List<ResolveInfo> all=getPackageManager().queryIntentActivities(it, 0); LinkedHashMap<String,ResolveInfo> map=new LinkedHashMap<>(); for(ResolveInfo ri:all){ if(!map.containsKey(ri.activityInfo.packageName)) map.put(ri.activityInfo.packageName,ri); } List<ResolveInfo> dedup=new ArrayList<>(map.values()); Collections.sort(dedup,(a,b)->a.loadLabel(getPackageManager()).toString().compareToIgnoreCase(b.loadLabel(getPackageManager()).toString())); allAppsCache=dedup; for(ResolveInfo ri:dedup){ try{ if(iconCache.get(ri.activityInfo.packageName)==null) iconCache.put(ri.activityInfo.packageName, ri.loadIcon(getPackageManager())); }catch(Exception e){} } mainH.post(()->{ setupDock(); if(rvFav!=null) rvFav.getAdapter().notifyDataSetChanged(); loadWallpaperFast(); }); }catch(Exception e){} }); }
    ...
    void makeDraggable_OLD(android.view.View v, int appId){
    ...
    void pickWidget_OLD(){ try{ int id=awHost.allocateAppWidgetId(); Intent pick=new Intent(android.appwidget.AppWidgetManager.ACTION_APPWIDGET_PICK); pick.putExtra(android.appwidget.AppWidgetManager.EXTRA_APPWIDGET_ID,id); startActivityForResult(pick, REQ_PICK_WIDGET); }catch(Exception e){} }
