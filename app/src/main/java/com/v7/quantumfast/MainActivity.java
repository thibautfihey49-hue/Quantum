...
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){
        ...
        preloadFast(); setupAtAGlanceSimple(); setupDock(); loadWallpaperFast(); setupGestures();
...
void saveFolders(){ StringBuilder sb=new StringBuilder(); for(int i=0;i<folders.size();i++){ if(i>0) sb.append(";;"); sb.append(folders.get(i).name).append("||").append(String.join(",",folders.get(i).pkgs)); } prefs.edit().putString("folders",sb.toString()).apply(); }
 ...
 void updateDockIcon(ImageView iv, String pkg){
    ...
    void launch(String pkg){ try{ android.content.Intent ii=getPackageManager().getLaunchIntentForPackage(pkg); if(ii!=null){ ii.addFlags(android.content.Intent.FLAG_ACTIVITY_REORDER_TO_FRONT); startActivity(ii); return; } }catch(Exception e){}  try{ trackUsage(pkg); Intent it=getPackageManager().getLaunchIntentForPackage(pkg); if(it!=null){ it.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK); startActivity(it);} }catch(Exception e){} }
